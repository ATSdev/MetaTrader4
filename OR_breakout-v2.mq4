...
string	ea_calledfrom	= "EA-OR_breakout-v2";
#include "libraries/lib_OpeningRange-v2.mq4"

extern	string	ORb_comment					= "OR_breakout-v2 settings below";

extern	int		orb_open_apply						= 1;
extern	int		orb_close_apply						= 1;
extern	int		orb_close_FIBO1						= 0;
...
int start() {
    ...
    for (bar = not_counted; bar >= 0; bar--) {
		...
		if (takeprofit_pips_apply == true) {
			if (ticket_long_current > 0 || signal_open_long == true) {
				takeprofit_0 = pips_takeprofit_calculation(takeprofit_pips, 0);
				if (takeprofit_0 > 0) mark_stop(takeprofit_0, Green);
			}
...
int set_global_as_external(int overwrite = 1) {
	...
	if (ret == 0) {
		set_datetime = GlobalVariableSet(globalvar_prefix + "stoploss_pips", stoploss_pips);
		if (set_datetime == 0) ret = GetLastError();
	}

	if (ret == 0) {
		set_datetime = GlobalVariableSet(globalvar_prefix + "takeprofit_pips", takeprofit_pips);
		if (set_datetime == 0) ret = GetLastError();
	}
